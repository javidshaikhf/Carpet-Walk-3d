...
namespace PathCreation {
    public class PathCreator : MonoBehaviour {
        ...
        IEnumerator CreateWaypointsForBezeirCurve()
        {
            Vector3 lastPos = carpetTransform.position;

            while (true)
            {
                yield return new WaitForSeconds(0.1f);
                
                if (Vector3.Distance(lastPos, carpetTransform.position) > 0.1)
                {
                    bezierPath.AddSegmentToEnd(carpetTransform.position);
                   
                }
                
                lastPos = carpetTransform.position;
            }
        ...
        private void Start()
        {
            carpetTransform = GameObject.FindWithTag("carpet").transform;
